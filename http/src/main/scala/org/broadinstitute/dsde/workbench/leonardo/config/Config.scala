...
object Config {
  ...
  val nonLeoMessageSubscriberConfig: SubscriberConfig = SubscriberConfig(
    applicationConfig.leoServiceAccountJsonFile.toString,
    topic,
    Some(ProjectSubscriptionName.of(applicationConfig.leoGoogleProject.value, "nonLeoMessageSubscription")),
    config.as[FiniteDuration]("pubsub.ackDeadLine"),
    Some(SubscriberDeadLetterPolicy(TopicName.of("broad-dsde-dev", "leonardo-pubsub-qi-test"), MaxRetries(5))),
    None,
